...
impl<'gc> Object<'gc> {
    ...
    pub(super) fn set_local(
        ...
        if handlers::is_hit_area(&name) {
            handlers::hit_area_assigned(self, value, |value| value.as_object(activation));
        }
    ...
    pub fn delete(self, activation: &mut Activation<'_, 'gc>, name: AvmString<'gc>) -> bool {
        ...
        if removed {
            if handlers::is_hit_area(&name) {
                handlers::hit_area_deleted(self);
            }
            handlers::property_changed(self, &name);
        }
        removed
    ...
    pub fn add_property(
        ...
        if handlers::is_hit_area(&name) {
            handlers::hit_area_virtual(self);
        }
        if inserted {
            handlers::property_changed(self, &name);
        }
    ...
    pub fn define_value(
        ...
        let name = name.into();
        if handlers::is_hit_area(&name) {
            handlers::hit_area_assigned(self, value, |_| None);
        }
