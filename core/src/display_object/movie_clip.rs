...
impl<'gc> TInteractiveObject<'gc> for MovieClip<'gc> {
    ...
    fn mouse_pick_avm1(
        ...
        if self.visible() {
            ...
            // hit outside its pick bounds, so the whole subtree is pruned here — unless
            // a clip in it has a `hitArea` that may lie outside its own subtree
            // (`unbounded_hit_areas`), or a prototype defines one.
            if self.unbounded_hit_areas() == 0
                && !crate::avm1::handlers::proto_hit_area()
                && !(local_to_global * self.pick_bounds()).contains(point)
            {
                return None;
            }
            ...
            if self.mouse_enabled() {
                ...
                if is_button_mode {
                    ...
                    // on the next pick).
                    let hit_area = if self.has_handler(context, Handler::HitArea) {
                        let area = self.avm1_hit_area(context);
                        if let Some(area) = area {
                            crate::avm1::handlers::hit_area_resolved(self.into(), area);
                        }
                        area.filter(|_| require_button_mode)
                    } else {
                        None
                    };
